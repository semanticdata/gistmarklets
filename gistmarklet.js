var load_gistmarklet = function () {
  ...
  $("#header input").val(id);
  $.getJSON(
    "https://api.github.com/gists/" + id + "?&callback=?",
    function (data) {
      for (var filename in data.data.files) break;
      script = data.data.files[filename].content;

      if (script.slice(0, 11) != "javascript:") script = "javascript:" + script;
      ...
      if (navigator.userAgent.indexOf("Chrome") > 0)
        $("#note").text(
          "You may need to press ctrl+b to show your bookmarks bar"
        );
      else if (navigator.userAgent.indexOf("Firefox") > 0)
        $("#note").text(
          "You may need to show your bookmarks bar. Firefox menu > preferences > Bookmarks Toolbar"
        );
...
var hashchange_func = $(document).ready(function () {
  $(window)
